...
class TestMarginFloor:
    ...
    def test_names_the_latest_quarter_when_it_set_the_floor(self):
        # A bound the newest quarter just set has no headroom; a reader who
        # cannot see that reads stability into a fresh deterioration.
        d = only(series(revenue=[100.0] * 4, operating_income=[20.0, 19.0, 18.0, 17.0]),
                 "margin_floor")
        assert d is not None and "the most recent" in d.text
...
class TestDilution:
    ...
    def test_falls_back_to_shares_outstanding_when_diluted_has_the_q4_hole(self):
        ...
        diluted = [100.0, 100.0, 100.0, None, 110.0, 110.0, 110.0, None]
        outstanding = [100.0, 100.0, 100.0, 100.0, 110.0, 110.0, 110.0, 110.0]
