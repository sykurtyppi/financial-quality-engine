...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    for p in paths:
        print(f"  {p.stem.replace('.json', '')}  {p.stat().st_size / 1024:8.0f} KB")
    return 0
...
def cmd_diff(args: argparse.Namespace) -> int:
    ...
    if len(paths) < 2:
        print(f"{args.ticker}: {len(paths)} snapshot(s) — a diff needs two. "
              ...
              file=sys.stderr)
        return 2
    by_day = {p.name.split(".")[0]: p for p in paths}
    ...
    changes = diff_vintages(
        load_vintage(older), load_vintage(newer),
        scored_only=not args.all_tags,
        since=date.fromisoformat(args.since) if args.since else None,
        include_split_adjusted=args.splits,
    )
    print(render_changes(changes, older.name.split(".")[0], newer.name.split(".")[0]))
    return 0
...
def main() -> int:
    ...
    d.add_argument("--all-tags", action="store_true",
                   help="every XBRL tag, not only the fields the engine scores")
