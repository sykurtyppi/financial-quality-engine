...
marker (`KNOWN_*` in `scripts/drill.py`). Current:

- **An amendment is also reported as a silent revision** (step 3, found
  2026-09-26). The vintage diff compares scored values between snapshots and
  does not ask whether a new filing explains a move. So the 10-Q/A produces
  both `Restatement (10-Q/A) …` and a Tier-1 `Silent revision: revenue …`
  line, and the appendix lists the row under "Nothing here has an amended
  filing behind it". The fix changes what the card promotes
  (`app/services/ingestion/vintages.py`), so it needs its own review.
